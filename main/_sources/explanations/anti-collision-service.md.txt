...
**Two questions, and no others.** The service owns the geometry; the checker
owns transforms, branches, soft limits and windows. That reduces the interface
to *given an axis, what can it ever collide with* and *given these axis moves,
do they collide*.

**A static reachability relation, axis to axis.** The first question is about
every position of every axis the model permits, not the current pose, so it is
answerable once from the geometry and the limit-switch positions. It exists to
bound the second: the checker gathers state only for axes that can ever interact
with the one moving. The machinery is already present — `buildAdjacencyPairs` in
`js/device.js:25` computes the pairs to skip, and the checked set is its
...
answering from its own idea of "current pose" would answer the wrong question
for every insertion-time check made while a scan is running. It also makes the
batch API a pure function of its request.
