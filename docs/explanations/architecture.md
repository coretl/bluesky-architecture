...
### What the checker asks the service, and what it keeps

The division is that the service owns the geometry and the checker owns
everything else — transforms, branches, soft limits, windows. That gives the
service exactly two questions to answer:

1. **Given an axis, what can it *ever* collide with?** Not at the current pose —
   over every position of every axis the model permits. This is statically
   knowable from the geometry and the limit-switch positions, with no knowledge
   of where anything is now.
2. **Given this set of axis moves, do they collide?**

The first is an **axis-to-axis relation**, and it is what makes the second
tractable: it bounds the state the checker has to gather. To check a move of one
axis, the checker needs the positions of the axes that can ever interact with
it, not the whole machine. The relation is expected to be trimmed by hand at
first.

The relation must never omit a pair that can touch; extra pairs only cost state
gathering. It is computed **after** the exemption set, so permanently-touching
pairs do not appear in it — which makes it depend on that set being settled, and
it is not. See [](open-questions.md).
...
limits are applied by the checker at validation time and the service never sees
them — so tightening a soft limit cannot silently invalidate a cached relation.

The checker is told the relation and nothing else about geometry. Which axes
have branches is not registered: it is a property of the device, read off the
transform.

### Windows and turnarounds

Iterating a scanspec2 `Scan` yields one `Window` per setpoint, so per-window
branch is per-point branch for step scans with no special case.
